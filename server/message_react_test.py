...
def test_channel_invalid_message_id(d):
    ...
    with pytest.raises(ValueError):
        message_react(dict['token'], None, 1)

@pytest.mark.parametrize("react_id", [0, 2, -1, 'Apple', ' ', None])
def test_channel_invalid_react_id(d, react_id):
    ...
    with pytest.raises(ValueError):
        message_react(dict['token'], message_id, react_id)

def test_message_remove_succestful(d):
